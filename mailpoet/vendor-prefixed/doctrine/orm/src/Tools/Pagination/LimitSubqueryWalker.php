...
class LimitSubqueryWalker extends TreeWalkerAdapter
{
 ...
 $query->setHint(self::IDENTIFIER_TYPE, Type::getType($rootClass->fieldMappings[$identifier]['type']));
 $query->setHint(self::FORCE_DBAL_TYPE_CONVERSION, \true);
 ...
 $AST->selectClause->isDistinct = ($query->getHints()[Paginator::HINT_ENABLE_DISTINCT] ?? \true) === \true;
 ...
 $AST->selectClause->selectExpressions[] = new SelectExpression($this->createSelectExpressionItem($item->expression), '_dctrn_ord' . $this->aliasCounter++);
 continue;
 }
