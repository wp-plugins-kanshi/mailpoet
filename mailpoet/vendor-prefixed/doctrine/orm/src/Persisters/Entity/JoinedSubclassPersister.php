...
class JoinedSubclassPersister extends AbstractEntityInheritancePersister
{
 ...
 return 'SELECT COUNT(*) ' . 'FROM ' . $tableName . ' ' . $baseTableAlias . $joinSql . (empty($conditionSql) ? '' : ' WHERE ' . $conditionSql);
 }
 ...
 $columnList[] = $this->getSelectJoinColumnSQL($tableAlias, $joinColumn['name'], $this->quoteStrategy->getJoinColumnName($joinColumn, $this->class, $this->platform), PersisterHelper::getTypeOfColumn($joinColumn['referencedColumnName'], $targetClass, $this->em));
 }
 }
 ...
 $columns = $this->class->parentClasses ? $this->class->getIdentifierColumnNames() : [];
 foreach ($this->class->reflFields as $name => $field) {
 if (isset($this->class->fieldMappings[$name]['inherited']) && !isset($this->class->fieldMappings[$name]['id']) || isset($this->class->associationMappings[$name]['inherited']) || $this->class->isVersioned && $this->class->versionField === $name || isset($this->class->embeddedClasses[$name]) || isset($this->class->fieldMappings[$name]['notInsertable'])) {
 continue;
 }
 ...
 } elseif ($this->class->name !== $this->class->rootEntityName || !$this->class->isIdGeneratorIdentity() || $this->class->identifier[0] !== $name) {
 ...
 $class = isset($column['inherited']) ? $this->em->getClassMetadata($column['inherited']) : $this->class;
 } else {
 continue;
 }
 ...
 $sql = 'SELECT ' . implode(', ', $columnNames) . ' FROM ' . $tableName . ' ' . $baseTableAlias . $joinSql . ' WHERE ' . implode(' = ? AND ', $identifier) . ' = ?';
